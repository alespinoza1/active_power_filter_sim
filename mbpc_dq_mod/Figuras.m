...
figure('Units','inches','Position',[03 03 6 2],'PaperPositionMode','auto');
...
%Error cuadratico medio de seguimiento
...
ECMa = sqrt(sum(power(IR_IC_abc.data(:,1)-IR_IC_abc.data(:,4),2))/((b-a)+1))
ECMb = sqrt(sum(power(IR_IC_abc.data(:,2)-IR_IC_abc.data(:,5),2))/((b-a)+1))
ECMc = sqrt(sum(power(IR_IC_abc.data(:,3)-IR_IC_abc.data(:,6),2))/((b-a)+1))

...
global fo fn Theta v_q_km1 ylfkm1 
...
if tact-tant >= Tm
    ...
    ica_ref=  sqrt(2/3)*(sin(Theta)*Idc + cos(Theta)*Iqc + 1*0 );
    icb_ref=  sqrt(2/3)*(sin(Theta-2*pi/3)*Idc + cos(Theta-2*pi/3)*Iqc +1*0);
    icc_ref=  sqrt(2/3)*(sin(Theta+2*pi/3)*Idc + cos(Theta+2*pi/3)*Iqc +1*0);
